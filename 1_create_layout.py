...
def merge_image():
    try:
        ...
        for idx, img in enumerate(images):
            ...
            progress = (idx +1) / len(images) *100  # 실제 percet 정보를 계산
            p_var.set(progress)
            progress_bar.update()

        file_name = "nado_photo."+img_format
        dest_path = os.path.join(txt_dext_path.get(), file_name)
...
scrollbar.pack(side="right", fill="y") # y는 위아래로 펼쳐지게 함
list_file = Listbox(list_frame, selectmode="extended", height = 15, yscrollcommand=scrollbar.set) # 15개까지 보임
...
# 3.파일 포멧 옵션
# -파일 포멧 레이블
lbl_format = Label(frame_option, text="가로넓이", width=8)
